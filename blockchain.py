...
class Block(object):
    ...
    def __init__(self, index, previous_hash, timestamp, data, hash,address):
        self.index = index
...
class Blockchain(object):
    ...
    def get_genesis_block():
        ...
        return Block(0, "0", 1465154705, "my genesis block!!",
                     "816534932c2b7154836da6afc367695e6337db8a921823784c14378abed4f7d7","peer21")
    ...
    def generate_next_block(self, block_data):
        ...
        next_hash = self.calculate_hash(next_index, previous_block.hash, next_timestamp, block_data["data"])
        return Block(next_index,previous_block.hash, next_timestamp, block_data["data"], next_hash,block_data["peer"])
    ...
    def calculate_hash(index, previous_hash, timestamp, data):
        ...
        hash_object = SHA256.new(data=(str(index) + previous_hash + str(timestamp) + str(data)).encode())
    ...
    def add_block_temp(self, new_block):

        if self.is_valid_new_block(new_block, self.get_latest_block()):
            self._blockchain_temp.append(new_block)
